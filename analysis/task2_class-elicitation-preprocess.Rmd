...
d.tidy.resp.valid.FailedRef <- d.tidy.resp.valid %>%
  ...
  mutate(failed_ref = response %in% incorrect.referent | 
          ...
          (superordinate == "venues" & 
             response %in% c("night", "nights", "saturday nights", "Saturday nights") ) |
          (superordinate == "places" & response %in% c("saturday", "saturdays")) |
          (superordinate == "vehicles" & response %in% c("highway", "highways")) |
          ...
          (superordinate == "writing utensils" & response %in% c("paper")) |
...
d.tidy.resp.valid.noFailedRef %>%
  ...
  mutate( # ITEM-SPECIFIC SYNONYMS
    response_synonym = case_when(
      ...
      (np == "jockey" & 
         response_synonym %in% c("player", "players")) ~ "jockeys",
      (np == "golfer" & 
         response_synonym %in% c("player", "players")) ~ "golfers",
      (np == "sitcom" &
         response_synonym %in% c("episode", "episodes") ) ~ "sitcom",
      (np == "podcast episode" & 
         response_synonym %in% c("episode", "episodes")) ~ "podcast episode",
      (np == "rock concert" & 
        response_synonym == "rock") ~ "rock concert",
      (np == "trash can" & 
        response_synonym == "cans") ~ "trash cans",
...
d.tidy.auto.non_match.grab3 %>%
  distinct(stim_id, superordinate, np, adj, response_hand_lemma, response) %>%
  arrange(stim_id, superordinate, np, adj, response_hand_lemma, response) -> d.tidy.auto.non_match.grab3_distinct
