...
with(d.tidy.auto, table(specific, matchingSuper)) -> d.auto.table


d.auto.table %>% 
  kable()
prop.table(d.auto.table)
...
```
broken down by 3 x 2
```{r}
d.tidy.auto %>%
  group_by(np_expectations, adj_polarity) %>%
  summarize(
    sub = sum(specific),
    super = sum(matchingSuper),
    other = sum(!specific & !matchingSuper),
    psub = sub / n(),
    psuper = super / n(),
    pother = other / n()
  )
...
nrow(d.tidy.auto.non_match.grab3)
...
```{r}
# write_csv(d.tidy.auto.non_match.grab3.2, "../data/task2_comparison-class-inference/class-inference_handCategorizedOthers.csv")
# 
# write_csv(d.tidy.auto.freq, "../data/task2_comparison-class-inference/class-inference_preprocessed_noInvalid_noRefFail.csv")

df.temp <- read_csv("../data/task2_comparison-class-inference/class-inference_handCategorizedOthers.csv")
```
