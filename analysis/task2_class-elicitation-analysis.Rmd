...
**Don't run: Load results in next chunk** Bootstrapping condition and item means. 

```{r boostrap condition and item CIs, eval = F}
...
#     d.tidy.phrase.specificMarking.bs.item, 
#     file = "cached_results/class-elicitation-bs-byItem-fullSet.RData")
```

## Fig 3: Comparison Class Inference experimental results. 

Proportion of paraphrases that contained the Subordinate NPs (e.g., basketball player) with which the referent was introduced, as a function of the general expectations listeners have about the category (x-axis) and the polarity of the adjective used to describe the category (e.g., tall = positive, short = negative). Bars represent overall means and error bar is a bootstrapped 95% confidence interval. Each dot represents the mean of a single item and lines connect Subordinate NPs described with different adjectives (e.g., tall and short basketball player). Dots are jittered horizontally to improve visual clarity.

```{r}
load(file = "cached_results/class-elicitation-bs-byItem-fullSet.RData") # d.tidy.phrase.specificMarking.bs, d.tidy.phrase.specificMarking.bs.item, file =
...
Comparison class inference results for 24 of the 90 items. Two examples were selected from each unique degree scale; example items were those that exhibited the greatest and smallest variability in comparison class inferences.

```{r figIndivItems}
select.items <- d.tidy.phrase.specificMarking.bs.item %>%
  select(degree, subordinate_sg, stim_id, superordinate_pl, np_expectations, adj_polarity, mean) %>%
  rowwise() %>%
  mutate(degree = first(strsplit(degree, "_")[[1]])) %>%
  spread(adj_polarity, mean) %>%
  mutate(delta = positive - negative) %>%
  ungroup() %>%
  group_by(degree, stim_id, superordinate_pl) %>%
...
d.tidy.phrase.specificMarking.bs.item %>%
  ...
  mutate(
    ...
    subordinate_sg = fct_reorder(subordinate_sg, as.numeric(np_expectations)),
    item_set = paste(degree, " - ", superordinate_pl, "-", stim_id, sep = ""),
    item_name = paste(superordinate_pl, " (", degree, ")", sep = ""),
  ...
  ggplot(
    .,
    aes(x = subordinate_sg, y = mean, fill = adj_polarity)
  ) +
...
# ggsave(
#   filename = "../paper/figs/3way_bars_cc_finalExpt.pdf",
#   width = 7, height = 3.2
# )
```
...
**Don't run: Load results in next chunk** 

```{r marginal posterior samples, eval = F}
...
#           path = "cached_results/rs_brm_glm_exact-match_4chain_10k_samples.csv")
```


```{r}
rs.glm.brm.betas.samples <- read_csv("cached_results/rs_brm_glm_exact-match_4chain_10k_samples.csv")
...
#           path = "../paper/csv_data_4_tex/expt_brm_exact-match_contrasts_10k_2.csv")
```
